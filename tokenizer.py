...
# inspired by https://github.com/moses-smt/mosesdecoder/blob/master/scripts/tokenizer/tokenizer.perl used in nmt's
# examples
...
with open(preprocessing['protected_phrases_file'], 'r', encoding='utf-8') as protected_file:
    protected_phrases_regex = list(filter(lambda word: False if word[0] == '#' else True,
                                          filter(None, protected_file.read().split("\n"))))
...
def tokenize(sentence):
    ...
    if protected_phrases_counter:
        sentence = regex['restorephrases'].sub(
            lambda number: protected_phrases_replace[int(number.group(1).replace(" ", ""))], sentence)
...
def replace(entity):
    ...
    phrase_ = list(filter(None, list(entity.groups())))[0]
    replacement = entity.group(0).replace(phrase_, ' PROTECTEDREGEXPHRASE{}PROTECTEDREGEXPHRASE '.format(
        protected_phrases_counter))
    protected_phrases_replace.append(phrase_)
...
with open(preprocessing['answers_detokenize_file'], 'r', encoding='utf-8') as answers_detokenize_file:
    answers_detokenize_regex = list(filter(lambda word: False if word[0] == '#' else True,
                                           filter(None, answers_detokenize_file.read().split("\n"))))
    ...
    for answer in answers:
        ...
        for detokenize_regex in answers_detokenize_regex:
            ...
            if re.search(detokenize_regex, answer):
                ...
                regex_ = re.compile(detokenize_regex)
                for p in regex_.finditer(answer):
