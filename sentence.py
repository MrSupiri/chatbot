...
with open(preprocessing['answers_blacklist_file'], 'r', encoding='utf-8') as answers_blacklist_file:
    answers_blacklist = list(filter(lambda word: False if word[0] == '#' else True,
                                    filter(None, answers_blacklist_file.read().split("\n"))))
with open(preprocessing['vocab_blacklist_file'], 'r', encoding='utf-8') as vocab_blacklist_file:
    vocab_blacklist = list(filter(lambda word: False if word[0] == '#' else True,
                                  filter(None, vocab_blacklist_file.read().split("\n"))))
...
with open(preprocessing['answers_replace_file'], 'r', encoding='utf-8') as answers_replace_file:
    answers_replace = list(
        filter(lambda word: False if word[0] == '#' else True, filter(None, answers_replace_file.read().split("\n"))))
with open(preprocessing['vocab_replace_file'], 'r', encoding='utf-8') as vocab_replace_file:
    vocab_replace = list(
        filter(lambda word: False if word[0] == '#' else True, filter(None, vocab_replace_file.read().split("\n"))))
